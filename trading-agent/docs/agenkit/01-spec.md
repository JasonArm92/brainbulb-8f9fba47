...
5. Size with Kelly on *our* calibrated p_win, net of costs, × min(fraction, 0.25).
...
7. Hard limits (code, frozen dataclass): max DD 15% (latched kill, reduce-only after), daily loss 3%,
   position 10%, gross 30%, order 5%, 50 bp price band, 5 s staleness, 20 orders/min, KILL file.
...
## Gate 1 checklist
- [ ] Every figure in `research/REPORT.md` re-pulled from a primary source (venue API, DefiLlama,
      Tokenomist, issuer ETF files) with a timestamp; conflicts resolved.
- [ ] Finalists and `allowed_directions` confirmed.
- [ ] Jev SDK surface verified against docs.typesafe.ai (`trader/jev_client.py::_call_sdk/_parse`).
